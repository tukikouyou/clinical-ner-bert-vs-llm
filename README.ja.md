...
|------|------|:----------:|:---------:|:------:|
| **UTH-BERT**（ファインチューニング） | encoder, 臨床事前学習 | **0.752** | 0.871 | 0.661 |
| **NICT-BERT**（ファインチューニング） | encoder, Wikipedia事前学習 | **0.742** | 0.861 | 0.651 |
| Qwen3.6 35B | LLM, ゼロショット | 0.288 | 0.446 | 0.212 |
| Llama3.3 70B | LLM, ゼロショット | 0.261 | 0.522 | 0.174 |
| Qwen3-30B-A3B | LLM, ゼロショット | 0.244 | 0.316 | 0.199 |
| LLM-jp-4 32B | LLM, ゼロショット(推論型) | 0.068 | 0.324 | 0.038 |
| GPT-OSS 120B | LLM, ゼロショット(推論型) | 0.040 | 0.606 | 0.021 |
| SIP-jmed-LLM-3 13B | LLM, ゼロショット(推論型・**医療**) | 0.012 | 0.119 | 0.006 |
...
### LLMのファインチューニング(QLoRA) — 順位が逆転する

同じLLMを、BERTと**同一の146文書の学習分割**でQLoRAファインチューニング
(SFT: プロンプト→gold JSON)し、同一指標で評価:

| 手法 | ゼロショット F1 | **FT後 F1** | Δ |
|------|:-----------:|:-----------:|:--:|
| **Llama-3.3 70B** | 0.261 | **0.822** *(最高)* | +0.561 |
| **SIP-jmed-LLM-3 13B**(医療・13Bのみ) | 0.012 *(ゼロショット最下位)* | **0.819** | **+0.807** |
| **Qwen3-30B-A3B** | 0.244 | **0.812** | +0.568 |
| **LLM-jp-4 32B** | 0.068 | **0.811** | +0.743 |
| **LLM-jp-3.1 1.8B** | ≈0 | **0.777** | +0.78 |
| UTH-BERT(FT済みencoder) | — | 0.752 | — |
| NICT-BERT(FT済みencoder) | — | 0.742 | — |

**ファインチューニングで順位が完全に逆転する。** ゼロショットではBERT(0.752)が全LLMを
上回り医療モデルが*最下位*(0.012)。QLoRA後は**5つのLLM全て(0.777–0.822)がBERTを上回る**。
1.8Bの小モデル(ゼロショット≈0)でも0.777に達する。**13BのSIP-jmed(0.819)が70BのLlama-3.3
(0.822)にほぼ並ぶ**——ドメイン事前学習で13Bが70Bに匹敵。
ゼロショットの差は**能力ではなくタスク/形式適応**の問題だった — 構造化出力を
出せるようになれば、LLMの豊かな表現(と医療モデルのドメイン知識)がBERTを上回る。
ゼロショットでは*足枷*だったドメイン事前学習(JSONを出せない推論モデル)が、
FT後は決定的な*強み*になる。

手法: QLoRA(4-bit NF4 + LoRA r=16, attention+MLP), 3エポック, 単一GPU。
[`llm/qlora_train.py`](llm/qlora_train.py), [`llm/build_sft_data.py`](llm/build_sft_data.py) 参照。
...
- **医療特化モデルがゼロショット最下位。** SIP-jmed-LLM-3(13B, 日本語臨床)は0.012で全LLM中最下位（ただしFT後は同率トップに）。
  これは*推論チューニング*モデルで延々と推論を続け、**12kトークン予算でも入力の
  75%でパース可能なJSONを出せない**ため。構造化出力ができなければドメイン知識は
  役に立たない。
- **「思考」型LLMは構造化抽出に不向き**で、下位に固まる(SIP-jmed 0.012,
  GPT-OSS 0.040, LLM-jp-4 0.068)。即座にJSONを強制(guided decoding)すると推論を
