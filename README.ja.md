...
|------|:-----------:|:-----------:|:--:|
| **SIP-jmed-LLM-3 13B**(医療) | 0.018 *(最下位)* | **0.857** *(最高)* | **+0.839** |
| **LLM-jp-3.1 1.8B** | ≈0 | **0.792** | +0.79 |
| UTH-BERT(FT済みencoder) | — | 0.725 | — |
| NICT-BERT(FT済みencoder) | — | 0.703 | — |
| LLM-jp-4 32B | 0.094 | *(学習中)* | |
| Qwen / Llama / GPT-OSS | 0.27–0.29 / 0.05 | *(保留)* | |

**ファインチューニングで順位が完全に逆転する。** ゼロショットではBERTが全LLMを
上回り医療モデルが*最下位*。QLoRA後はLLMがBERTを上回り、医療モデルが*最高*
(0.857 vs BERTの0.725)。1.8Bの小モデル(ゼロショット≈0)でも0.792に達する。
