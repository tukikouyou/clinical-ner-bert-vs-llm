...
def train_val_loop_ner(train_vecs, ner_train_labels,
    ...
    best_epoch = -1
    epochs_no_improve = 0
    patience = getattr(args, "patience", 0)  # 0 なら早停なし
    t_start = time.time()
    # モデルを定義
    ...
    loss_dct = {"epoch": [], "train_NER_loss": [], "val_NER_loss": [], "val_F": [],
                "epoch_sec": [], "best_epoch": None, "best_val_F": None,
                "stopped_early": False, "total_sec": None}
    ...
    for epoch in range(args.max_epoch):
        ...
        # 評価
        ner_preds_decode = decode_ner_pipeline(model, ner_preds, tag2idx, val_vecs)
        ...
        loss_dct["epoch_sec"].append(round(time.time() - t_epoch, 1))

        # ベスト更新 & Early Stopping
        if val_F > best_val_F:
            ...
            if patience and epochs_no_improve >= patience:
                logger.info("Early stopping: {0}エポック改善なし(best={1}@epoch{2})".format(
                    epochs_no_improve, best_val_F, best_epoch))
                loss_dct["stopped_early"] = True
                break

    loss_dct["best_epoch"] = best_epoch
    loss_dct["best_val_F"] = best_val_F
    loss_dct["total_sec"] = round(time.time() - t_start, 1)
...
def test_loop_ner(X_test, test_vecs, ner_test_labels, fold, tag2idx, args, device):
    ...
    with torch.inference_mode():
        ...
        for ofs in range(0, len(test_vecs), args.batch_size):
            ...
            ner_preds.append(ner_logits)
    # 評価
    ner_preds_decode = decode_ner_pipeline(model, ner_preds, tag2idx, test_vecs)
    res_df = result2df_for_ner(X_test, ner_preds_decode)
    return res_df

def no_tag_test_loop_ner(X_test, test_vecs, fold, tag2idx, args, device):
    # テスト
    with torch.inference_mode():
        # モデルを定義
        model = BERT_CRF(args, tag2idx).to(device)
