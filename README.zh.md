...
| 方法 | 零样本 F1 | **微调后 F1** | Δ |
|------|:--------:|:------------:|:--:|
| **SIP-jmed-LLM-3 13B**（医学）| 0.018 *(最差)* | **0.857** *(最好)* | **+0.839** |
| **LLM-jp-3.1 1.8B** | ≈0 | **0.792** | +0.79 |
| UTH-BERT（微调编码器）| — | 0.725 | — |
| NICT-BERT（微调编码器）| — | 0.703 | — |
| LLM-jp-4 32B | 0.094 | *(训练中)* | |
| Qwen / Llama / GPT-OSS | 0.27–0.29 / 0.05 | *(待补)* | |

**微调让整个排名反转。** 零样本时 BERT 超过所有 LLM、医学模型最差；QLoRA 微调后
LLM 反超 BERT、医学模型最好（0.857 vs BERT 的 0.725）。哪怕 1.8B 的小模型
（零样本≈0）也到 0.792。零样本的差距是**任务/格式适配问题，不是能力问题**——
一旦模型学会产出结构化输出，LLM 更丰富的表示（加上医学模型的领域知识）就超过了
BERT。零样本时是*负担*的领域预训练（吐不出 JSON 的推理模型），微调后变成决定性
的*优势*。
